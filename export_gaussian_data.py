...
output_path = _resolve_path("./data/gaussian_data")
...
with open(_resolve_path("data_config.csv"), newline="", encoding="utf-8") as csvfile:
    ...
    for row in reader:
        ...
        for i in range(3):
            ...
            os.system(
                f"python {_resolve_path('./data_process/image_upscale.py')} --img_path {base_path}/{case_name}/color/{i}/0.png --output_path {output_path}/{case_name}/{i}_high.png --category {category}"
            )
            # Prepare the segmentation mask of the high-resolution image
            os.system(
                f"python {_resolve_path('./data_process/segment_util_image.py')} --img_path {output_path}/{case_name}/{i}_high.png --TEXT_PROMPT {category} --output_path {output_path}/{case_name}/mask_{i}_high.png"
            )
            ...
            os.system(
                f"python {_resolve_path('./data_process/segment_util_image.py')} --img_path {output_path}/{case_name}/{i}.png --TEXT_PROMPT 'human' --output_path {output_path}/{case_name}/mask_human_{i}.png"
            )
            os.system(
                f"python {_resolve_path('./data_process/segment_util_image.py')} --img_path {output_path}/{case_name}/{i}_high.png --TEXT_PROMPT 'human' --output_path {output_path}/{case_name}/mask_human_{i}_high.png"
            )
