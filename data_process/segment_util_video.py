...
parser.add_argument("--base_path", type=str, default=_resolve_path("./data/real_collect"))
...
GROUNDING_DINO_CONFIG = _resolve_path("./data_process/groundedSAM_checkpoints/GroundingDINO_SwinT_OGC.py")
GROUNDING_DINO_CHECKPOINT = _resolve_path("./data_process/groundedSAM_checkpoints/groundingdino_swint_ogc.pth")
BOX_THRESHOLD = 0.35
TEXT_THRESHOLD = 0.25
...
sam2_checkpoint = _resolve_path("./data_process/groundedSAM_checkpoints/sam2.1_hiera_large.pt")
_sam2_pkg_cfg = Path(sam2.__file__).resolve().parent / "configs" / "sam2.1" / "sam2.1_hiera_l.yaml"
if _sam2_pkg_cfg.exists():
    model_cfg = str(_sam2_pkg_cfg)
else:
    model_cfg = str(Path(_resolve_path("configs")) / "sam2.1" / "sam2.1_hiera_l.yaml")
