# Demo V1: Single-D455 Realtime Viewer

This folder contains the standalone operator entrypoint for the single-camera
demo. It streams one Intel RealSense D455 and renders either:
...
```bash
conda run -n FFS-SAM-RS python demo_v1/realtime_single_camera_pointcloud.py \
  --profile 848x480 \
  --fps 60 \
...
```bash
./demo_v1/run_wslg_open3d.sh conda run -n FFS-SAM-RS python demo_v1/realtime_single_camera_pointcloud.py --view-mode orbit
```
