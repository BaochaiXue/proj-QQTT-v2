...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render static round1-3 frame-0 object-only 3x3 PCD boards comparing "
            "native depth, original FFS, and per-pixel native/FFS fused depth."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Erode the object mask inward by this many 3x3 iterations before building PCDs.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Disable the default display-only radius-neighbor point-cloud cleanup.",
    )
...
def main() -> int:
    ...
    print(f"Native/FFS fused object PCD outputs written to {summary['output_dir']}")
