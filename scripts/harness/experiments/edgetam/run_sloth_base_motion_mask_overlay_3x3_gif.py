        ...
        for checkpoint_key, checkpoint_label, checkpoint_file, config in SAM21_SPECS:
            ...
            for camera_idx in (0, 1, 2):
                ...
                command = [
                    ...
                    "python",
                    str(ROOT / "scripts/harness/experiments/sam/run_sam21_checkpoint_ladder_3x5_gifs.py"),
                    "--worker",
                    "--case-key",
                    SLOTH_BASE_MOTION_CASE_KEY,
                    "--case-dir",
    ...
    if not bool(args.reuse_existing) and not bool(args.skip_edgetam):
        for camera_idx in (0, 1, 2):
            ...
            command = [
                ...
                "python",
                str(ROOT / "scripts/harness/experiments/edgetam/run_edgetam_video_masks.py"),
                "--case-key",
                SLOTH_BASE_MOTION_CASE_KEY,
                "--case-dir",
                str(case_dir),
