...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render static round1-3 frame-0 object-only 10x3 Open3D PCD boards comparing "
            "native depth, original FFS, and original FFS with object masks eroded from 1 to 8 pixels."
        )
    )
    parser.add_argument("--aligned_root", type=Path, default=ROOT / "data")
    parser.add_argument("--output_root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--frame_idx", type=int, default=0)
    parser.add_argument("--erode_pixels", type=str, default=DEFAULT_ERODE_PIXELS)
...
def main() -> int:
    ...
    print(f"FFS mask erode sweep object PCD experiment written to {summary['output_dir']}")
