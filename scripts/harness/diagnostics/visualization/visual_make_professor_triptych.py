...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a three-figure professor-facing Native-vs-FFS comparison pack.")
    ...
    parser.add_argument("--no_write_video", dest="write_video", action="store_false")
    parser.add_argument("--write_keyframes", dest="write_keyframes", action="store_true")
    parser.add_argument("--no_write_keyframes", dest="write_keyframes", action="store_false")
    parser.set_defaults(
...
def main() -> int:
    args = parse_args()
    from data_process.visualization.workflows.professor_triptych import run_professor_triptych_workflow

    output_dir = args.output_dir
    if output_dir is None:
        if args.case_name:
            output_dir = args.aligned_root / args.case_name / f"professor_triptych_frame_{args.frame_idx:04d}"
        else:
            output_dir = args.aligned_root / f"professor_triptych_{args.realsense_case}_vs_{args.ffs_case}_frame_{args.frame_idx:04d}"
    ...
    result = run_professor_triptych_workflow(
        ...
        write_keyframes=args.write_keyframes,
    )
    print(f"Professor triptych written to {result['output_dir']}")
    return 0
