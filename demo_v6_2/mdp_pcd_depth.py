...
def _transform_points_c2w(
    points_xyz_m: np.ndarray, c2w: np.ndarray | None
) -> np.ndarray:
...
def _world_z_class_stats(
    ...
    *,
    ...
    points = np.asarray(points_xyz_m, dtype=np.float32).reshape(-1, 3)
    finite = (
        np.isfinite(points).all(axis=1) if len(points) else np.zeros((0,), dtype=bool)
    )
...
def _select_points_by_yx_mask(
    points_xyz_m: np.ndarray, yx: np.ndarray, mask: np.ndarray
) -> np.ndarray:
    ...
    cols = coords[:count, 1]
    valid = (
        (rows >= 0) & (rows < target.shape[0]) & (cols >= 0) & (cols < target.shape[1])
    )
