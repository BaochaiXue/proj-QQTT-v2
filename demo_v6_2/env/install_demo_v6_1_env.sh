...
MAIN_ENV="${DEMO_V6_1_MAIN_ENV:-demo_2_max}"
SHAPE_ENV="${DEMO_V6_1_SHAPE_PRIOR_ENV:-phystwin-max}"
MODE="${1:-update}"
SAM3_PACKAGE_REF="${DEMO_V6_1_SAM3_PACKAGE_REF:-sam3==0.1.4}"
PYTORCH3D_SOURCE="${DEMO_V6_1_PYTORCH3D_SOURCE:-${HOME}/external/pytorch3d-demo-v6-1-cu126}"
PYTORCH3D_REF="${DEMO_V6_1_PYTORCH3D_REF:-v0.7.9}"
...
Environment variables:
  ...
  DEMO_V6_1_SHAPE_PRIOR_ENV  default: phystwin-max
  DEMO_V6_1_SAM3_PACKAGE_REF default: sam3==0.1.4
  DEMO_V6_1_PYTORCH3D_SOURCE default: ~/external/pytorch3d-demo-v6-1-cu126
  DEMO_V6_1_PYTORCH3D_REF    default: v0.7.9
...
install_main_compiled_extras() {
    ...
    local build_env=(
        ...
        FORCE_CUDA=1
        TORCH_CUDA_ARCH_LIST="${DEMO_V6_1_TORCH_CUDA_ARCH_LIST:-8.9}"
        MAX_JOBS="${DEMO_V6_1_BUILD_MAX_JOBS:-8}"
    )
    ensure_pytorch3d_source
...
main() {
    case "${MODE}" in
        ...
        check)
            ...
            ;;
        update|create)
            create_or_update_env "${MAIN_ENV}" "${SCRIPT_DIR}/environment-demo-v6-1-main.yml"
            install_pip_extras "${MAIN_ENV}" "${SCRIPT_DIR}/requirements-demo-v6-1-main.txt"
            install_main_no_deps_extras
            install_main_compiled_extras
            create_or_update_env "${SHAPE_ENV}" "${SCRIPT_DIR}/environment-demo-v6-1-shape-prior.yml"
            install_pip_extras "${SHAPE_ENV}" "${SCRIPT_DIR}/requirements-demo-v6-1-shape-prior.txt"
            install_shape_prior_compiled_extras
            run_checks
            ;;
        *)
