...
parser.add_argument(
    "--base_path",
    type=str,
    default=_resolve_path("./data/different_types"),
)
...
if PROCESS_SEG:
    ...
    with Timer("Video Segmentation"):
        os.system(
            f"python {_resolve_path('./data_process/segment.py')} --base_path {base_path} --case_name {case_name} --TEXT_PROMPT {TEXT_PROMPT}"
        )
...
if PROCESS_SHAPE_PRIOR and SHAPE_PRIOR:
    ...
    with Timer("Image Upscale"):
        os.system(
            f"python {_resolve_path('./data_process/image_upscale.py')} --img_path {base_path}/{case_name}/color/0/0.png --mask_path {mask_path} --output_path {base_path}/{case_name}/shape/high_resolution.png --category {category}"
        )

    # Get the masked image of the object
    with Timer("Image Segmentation"):
        os.system(
            f"python {_resolve_path('./data_process/segment_util_image.py')} --img_path {base_path}/{case_name}/shape/high_resolution.png --TEXT_PROMPT {category} --output_path {base_path}/{case_name}/shape/masked_image.png"
        )

    with Timer("Shape Prior Generation"):
        os.system(
            f"python {_resolve_path('./data_process/shape_prior.py')} --img_path {base_path}/{case_name}/shape/masked_image.png --output_dir {base_path}/{case_name}/shape"
        )
...
if PROCESS_TRACK:
    ...
    with Timer("Dense Tracking"):
        os.system(
            f"python {_resolve_path('./data_process/dense_track.py')} --base_path {base_path} --case_name {case_name}"
        )
...
if PROCESS_3D:
    ...
    with Timer("Lift to 3D"):
        os.system(
            f"python {_resolve_path('./data_process/data_process_pcd.py')} --base_path {base_path} --case_name {case_name}"
        )

    # Further process and filter the noise of object and controller masks
    with Timer("Mask Post-Processing"):
        os.system(
            f"python {_resolve_path('./data_process/data_process_mask.py')} --base_path {base_path} --case_name {case_name} --controller_name {CONTROLLER_NAME}"
        )

    # Process the data tracking
    with Timer("Data Tracking"):
        os.system(
            f"python {_resolve_path('./data_process/data_process_track.py')} --base_path {base_path} --case_name {case_name}"
        )
...
if PROCESS_ALIGN and SHAPE_PRIOR:
    ...
    with Timer("Alignment"):
        os.system(
            f"python {_resolve_path('./data_process/align.py')} --base_path {base_path} --case_name {case_name} --controller_name {CONTROLLER_NAME}"
        )
...
if PROCESS_FINAL:
    ...
    with Timer("Final Data Generation"):
        if SHAPE_PRIOR:
            os.system(
                f"python {_resolve_path('./data_process/data_process_sample.py')} --base_path {base_path} --case_name {case_name} --shape_prior"
            )
        else:
            os.system(
                f"python {_resolve_path('./data_process/data_process_sample.py')} --base_path {base_path} --case_name {case_name}"
            )
