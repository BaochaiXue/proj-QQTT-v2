...
class PhysDynamicModule:
    def __init__(
        ...
        cfg.episode_path = _resolve_path(f"./{episode_name}")
...
if __name__ == "__main__":
    ...
    init_pcd_path = _resolve_path(f"./{episode_name}/pcd_clean_new/000000.npz")
    ...
    rollout_dir = _resolve_path(f"./{episode_name}/robot")
    controller_xyzs = []
    controller_rots = []
    ...
    dynamic_module = PhysDynamicModule(
        base_path=_resolve_path("../gaussian_data"),
        case_name=case_name,
        experiments_path=_resolve_path("experiments"),
        experiments_optimization_path=_resolve_path("experiments_optimization"),
        output_dir=_resolve_path("temp_experiments"),
        init_pts=init_pts,
    ...
    best_model_path = glob.glob(_resolve_path(f"experiments/{case_name}/train/best_*.pth"))[0]

    exp_name = 'init=hybrid_iso=True_ldepth=0.001_lnormal=0.0_laniso_0.0_lseg=1.0'
    gaussians_path = _resolve_path(
        f"./gaussian_splatting/output/{case_name}/{exp_name}/point_cloud/iteration_10000/point_cloud.ply"
    )
