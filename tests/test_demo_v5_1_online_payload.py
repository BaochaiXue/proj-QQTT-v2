...
class DemoV51OnlinePayloadTest(unittest.TestCase):
    def test_chunk_payload_builder_is_memory_only(self) -> None:
        ...
        self.assertFalse(hasattr(writer, "write_data_process_chunk_case"))
        self.assertFalse(hasattr(writer, "validate_data_process_case"))
        final_data, track_process, manifest = writer.build_data_process_chunk_payload(
            _minimal_chunk(writer)
        )

        self.assertIn("object_points", final_data)
        self.assertIn("controller_points", final_data)
        self.assertEqual(final_data["query_schema_hash"], track_process["query_schema_hash"])
    ...
    def test_online_writer_does_not_emit_data_process_chunk_case(self) -> None:
        ...
        final_data, track_process, _ = writer.build_data_process_chunk_payload(
            _minimal_chunk(writer)
        )
