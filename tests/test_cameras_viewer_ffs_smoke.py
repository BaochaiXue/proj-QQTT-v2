...
class CamerasViewerFfsSmokeTest(unittest.TestCase):
    ...
    def test_parse_args_rejects_strict_batch_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir)
            repo = root / "ffs_repo"
            repo.mkdir()
            with mock.patch(
                "sys.argv",
                ["cameras_viewer_FFS.py", "--ffs_repo", str(repo), "--ffs_batch_mode", "unsupported"],
            ):
    ...
    def test_validate_ffs_batch_mode_args_rejects_non_off_mode(self) -> None:
        with self.assertRaisesRegex(ValueError, "only supports off"):
            _validate_ffs_batch_mode_args(worker_mode="per_camera", batch_mode="unsupported")

    def test_validate_ffs_batch_mode_active_camera_count_rejects_non_off_mode(self) -> None:
        with self.assertRaisesRegex(ValueError, "only supports off"):
            _validate_ffs_batch_mode_active_camera_count(batch_mode="unsupported", active_camera_count=2)
