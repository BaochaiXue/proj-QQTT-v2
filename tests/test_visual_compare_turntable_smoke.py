...
class VisualCompareTurntableSmokeTest(unittest.TestCase):
    def test_turntable_workflow_writes_primary_single_frame_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue((output_dir / "hero_compare_geom.png").is_file())
            self.assertTrue((output_dir / "hero_compare_rgb.png").is_file())
            ...
            self.assertTrue((output_dir / "turntable_keyframes_source.png").is_file())
            self.assertTrue((output_dir / "turntable_keyframes_source_split.png").is_file())
            self.assertTrue((output_dir / "turntable_keyframes_mismatch.png").is_file())
            self.assertTrue((output_dir / "orbit_compare_geom.mp4").is_file())
            self.assertTrue((output_dir / "orbit_compare_geom.gif").is_file())
            self.assertTrue((output_dir / "orbit_compare_rgb.mp4").is_file())
            self.assertTrue((output_dir / "orbit_compare_rgb.gif").is_file())
            self.assertTrue((output_dir / "orbit_compare_support.mp4").is_file())
            self.assertTrue((output_dir / "orbit_compare_support.gif").is_file())
            self.assertTrue((output_dir / "orbit_compare_source.mp4").is_file())
            self.assertTrue((output_dir / "orbit_compare_source.gif").is_file())
            self.assertTrue((output_dir / "orbit_compare_mismatch.mp4").is_file())
            self.assertTrue((output_dir / "orbit_compare_mismatch.gif").is_file())
            self.assertTrue((output_dir / "support_metrics.json").is_file())
            self.assertTrue((output_dir / "source_metrics.json").is_file())
            self.assertTrue((output_dir / "mismatch_metrics.json").is_file())
            self.assertTrue((output_dir / "source_attribution_legend.png").is_file())
            ...
            self.assertEqual(len(list((output_dir / "frames_source").glob("*.png"))), 3)
            self.assertEqual(len(list((output_dir / "frames_mismatch").glob("*.png"))), 3)
            self.assertEqual(len(list((output_dir / "frames_source_split").glob("*.png"))), 3)
            ...
            self.assertIn("source", metadata["outputs"])
            self.assertIn("source_split", metadata["outputs"])
            self.assertIn("mismatch", metadata["outputs"])
            self.assertEqual(metadata["hero_outputs"]["geom"], str((output_dir / "hero_compare_geom.png").resolve()))
            self.assertEqual(metadata["hero_outputs"]["rgb"], str((output_dir / "hero_compare_rgb.png").resolve()))
            self.assertIsNotNone(metadata["hero_angle_deg"])
            self.assertEqual(metadata["outputs"]["geom"]["hero_image_path"], str((output_dir / "hero_compare_geom.png").resolve()))
            self.assertEqual(metadata["outputs"]["rgb"]["hero_image_path"], str((output_dir / "hero_compare_rgb.png").resolve()))
            self.assertTrue(metadata["outputs"]["geom"]["gif_path"])
            self.assertTrue(metadata["outputs"]["rgb"]["gif_path"])
            self.assertTrue(metadata["outputs"]["support"]["gif_path"])
            self.assertTrue(metadata["outputs"]["source"]["gif_path"])
            self.assertTrue(metadata["outputs"]["mismatch"]["gif_path"])
