...
class EnhancedPhystwinRemovedOverlaySmokeTest(unittest.TestCase):
    ...
    def test_build_board_accepts_ir_pair_6x3_matrix(self) -> None:
        image_rows = [[np.full((40, 60, 3), 90, dtype=np.uint8) for _ in range(3)] for _ in range(6)]
        board = build_enhanced_phystwin_removed_overlay_board(
            round_label="Projector Off",
            frame_idx=0,
            model_config={
                "highlight_scope": "all",
                "phystwin_radius_m": 0.01,
        ...
        self.assertEqual(board.ndim, 3)
        self.assertGreater(board.shape[0], 6 * 40)
        self.assertGreater(board.shape[1], 220 + 3 * 60)
    ...
    def test_workflow_writes_6x3_ir_pair_overlay_and_summary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_root = Path(tmp_dir)
            aligned_root = tmp_root / "data"
            static_root = aligned_root / "static"
            ...
            round_summary = summary["rounds"][0]
            self.assertTrue(Path(round_summary["board_path"]).is_file())
            self.assertIn("6x3_ir_pair", Path(round_summary["board_path"]).name)
            self.assertEqual(round_summary["model_config"]["native_row_mode"], "ir_pair")
            self.assertEqual(round_summary["render_contract"]["native_row_mode"], "ir_pair")
            self.assertEqual(
                round_summary["render_contract"]["rows"],
                "rgb_mask_ir_left_ir_right_pcd_removed_ffs_depth_removed_rgb_removed",
            )
            self.assertEqual(len(render_collector.calls), 3)
            self.assertTrue(all(item["native_masked_valid_pixel_count"] == 0 for item in round_summary["per_camera"]))
            self.assertTrue(
                all(item["native_depth_info"].get("native_depth_row_disabled") for item in round_summary["per_camera"])
            )
