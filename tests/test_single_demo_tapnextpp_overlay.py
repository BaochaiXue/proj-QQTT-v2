...
class SingleDemoTapNextOverlayTest(unittest.TestCase):
    ...
    def test_headless_capture_writer_saves_filtered_pcd_depth_and_query_payloads(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("prepared_phystwin_frame_path", rows[0])
            prepared_path = output_dir / rows[0]["prepared_phystwin_frame_path"]
            self.assertTrue(prepared_path.is_file())
            prepared = demo.load_prepared_phystwin_frame(prepared_path)
            self.assertEqual(prepared.seq, 0)
            self.assertEqual(prepared.rgb_frame.shape, (4, 4, 3))
            self.assertEqual(prepared.pcd_points.shape, (1, 4, 4, 3))
            np.testing.assert_array_equal(prepared.query_points_yx, query_points_yx)
