"""Client for the remote FFS depth service (vendored for demo_v6_1).

Extracted from ``services/ffs_remote/ffs_depth_client.py`` so demo_v6_1 does not
import the repo-level ``services`` package. Only the client path is vendored:
``FfsRemoteDepthResult`` and ``FfsRemoteDepthClient``. The original module's
CLI/benchmark/RealSense machinery (and its ``pyrealsense2``/``PIL``/``argparse``
dependencies) is intentionally dropped. ``zmq`` remains a lazy in-method import,
so this module loads with only numpy present — the ``ffs_remote`` depth source
...
from demo_v6_1.utils.ffs_remote_protocol import (
    COMPRESSION_MODES,
    RETURN_TYPES,
    SPARSE_RETURN_TYPES,
...
__all__ = ["FfsRemoteDepthClient", "FfsRemoteDepthResult"]
