"""Wire protocol for the remote FFS depth service (vendored for demo_v6_1).

Vendored verbatim from ``services/ffs_remote/protocol.py`` so demo_v6_1 does
not import the repo-level ``services`` package. Self-contained: only numpy and
stdlib at module scope; the zstd/lz4/PIL codecs stay lazy in-function.
"""
