...
from demo_v6_1.chunk_data_output import ChunkDataWriter
...
from demo_v6_1 import asap
from demo_v6_1 import phystwin_strict_product as strict
from demo_v6_1 import tracking
...
def _apply_radius_outlier_to_mask_frame(
    ...
    *,
    ...
    # corresponding mask pixels. Demo v6.1 does the same per frame in memory.
    normalized = strict.normalize_processed_mask_frame(frame)
    if not bool(enabled):
        return normalized
...
def _write_chunk_from_rows(
    *,
    ...
    if prepared and len(prepared) == len(rows) and prepared_count == len(rows):
        ...
        # Fallback for historical captures where Demo v6.1 has to reconstruct
