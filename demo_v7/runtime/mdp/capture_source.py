...
from demo_v7.runtime.mdp.constants import DEFAULT_PROFILE, DEPTH_SOURCES, _resolve_path
...
from demo_v7.runtime.utils.camera import (
    CameraIntrinsics,
    apply_emitter,
...
class LiveLatestFrameSampler:
    ...
    def __init__(self, sample_fps: float) -> None:
        """Initialize the sampler for a positive output frame rate."""
...
class RecordedRgbdFrameSource:
        ...
        *,
        ...
        if not self.metadata_path.is_file():
            raise FileNotFoundError(
                f"recording metadata not found: {self.metadata_path}"
            )
        try:
            metadata = json.loads(self.metadata_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError as exc:
            raise ValueError(
                f"recording metadata is not valid JSON: {self.metadata_path}"
            ) from exc
        ...
        if self.requires_ir and not {"ir_left", "ir_right"}.issubset(streams_present):
            raise ValueError(
                "FFS fake-live replay requires streams_present to include ir_left and ir_right"
            )
        ...
        if not isinstance(camera_recording, dict) or not camera_recording:
            raise ValueError(
                f"recording metadata has no frames for camera {self.camera_index}"
            )
        self.k_color = self._camera_matrix(
            metadata, "K_color", fallback_key="intrinsics"
        )
        """
        K = [ fx   0  cx ]
        [  0  fy  cy ]
        ...
        self.depth_scale_m_per_unit = self._camera_float(
            metadata, "depth_scale_m_per_unit"
        )
        self.serial = self._camera_string(
            metadata, "serial_numbers", default=f"recording-cam{self.camera_index}"
        )
        ...
        if {"ir_left", "ir_right"}.issubset(streams_present):
            try:
                ...
                self.t_ir_left_to_color = self._camera_transform(
        ...
        if self.requires_ir and not self.has_ir_stereo:
            raise ValueError(
                "FFS fake-live replay requires IR stereo calibration in metadata"
            )
    ...
    def read_packet(
        ...
        *,
        ...
        depth_u16 = (
            self._load_depth_u16(ref.depth_path) if ref.depth_path is not None else None
        )
        ir_left_u8 = (
            self._load_gray_u8(ref.ir_left_path)
            if ref.ir_left_path is not None
            else None
        )
        ir_right_u8 = (
            self._load_gray_u8(ref.ir_right_path)
            if ref.ir_right_path is not None
            else None
        )
        ...
        copy_ms = (
            _elapsed_ms(copy_start_s, copy_done_s)
            if frame_copy_ms is None
            else float(frame_copy_ms)
        )
        return FramePacket(
            seq=packet_seq,
            color_bgr=color_bgr,
            ...
            timing=PipelineTiming(
                wait_ms=float(wait_ms), align_ms=0.0, frame_copy_ms=copy_ms
            ),
            depth_u16=depth_u16,
    ...
    def _camera_matrix(
        self, metadata: dict[str, Any], key: str, *, fallback_key: str | None = None
    ) -> np.ndarray:
        ...
        if (
            ...
            or self.camera_index >= len(values)
            or values[self.camera_index] is None
        ):
            raise ValueError(
                f"recording metadata missing {key} for camera {self.camera_index}"
            )
        matrix = np.asarray(values[self.camera_index], dtype=np.float32)
        if matrix.shape != (3, 3):
            raise ValueError(
                f"recording metadata {key}[{self.camera_index}] must be 3x3"
            )
        if float(matrix[0, 0]) <= 0.0 or float(matrix[1, 1]) <= 0.0:
            raise ValueError(
                f"recording metadata {key}[{self.camera_index}] has non-positive focal length"
            )
    ...
    def _camera_transform(self, metadata: dict[str, Any], key: str) -> np.ndarray:
        ...
        if matrix.shape != (4, 4):
            raise ValueError(
                f"recording metadata {key}[{self.camera_index}] must be 4x4"
            )
        return np.ascontiguousarray(matrix, dtype=np.float32)
    ...
    def _camera_baseline(self, metadata: dict[str, Any]) -> float:
        ...
        if (
            ...
            and self.camera_index < len(values)
            and values[self.camera_index] is not None
        ):
            value = float(values[self.camera_index])
            if value <= 0.0:
                raise ValueError(
                    f"recording metadata ir_baseline_m[{self.camera_index}] must be positive"
                )
            return value
        ...
        if baseline <= 0.0:
            raise ValueError(
                "recording metadata T_ir_left_to_right"
                f"[{self.camera_index}] has non-positive baseline"
            )
        return baseline
    ...
    def _camera_float(self, metadata: dict[str, Any], key: str) -> float:
        ...
        if value <= 0.0:
            raise ValueError(
                f"recording metadata {key}[{self.camera_index}] must be positive"
            )
        return value

    def _camera_string(
        self, metadata: dict[str, Any], key: str, *, default: str
    ) -> str:
    ...
    def _build_recording_elapsed_s(
        ...
        timestamps = np.asarray(
            [float(frame.timestamp_s) for frame in frames], dtype=np.float64
        )
        if (
            len(timestamps)
        ...
        return np.ascontiguousarray(
            frame_indices / float(self.recording_fps), dtype=np.float64
        )
    ...
    def source_index_for_recording_elapsed_s(self, elapsed_s: float) -> int:
        ...
        index = int(
            np.searchsorted(self._recording_elapsed_s, elapsed + 1e-9, side="right") - 1
        )
        return max(0, min(index, len(self.frames) - 1))

    def _build_frame_refs(
        self, camera_recording: dict[str, Any]
    ) -> list[RecordedRgbdFrameRef]:
        """Build frame refs."""
        ...
        for step_text, timestamp in sorted(
            camera_recording.items(), key=lambda item: int(item[0])
        ):
            step = int(step_text)
            ...
            if self.requires_ir:
                if not ir_left_path.is_file():
                    raise FileNotFoundError(
                        f"recording IR left frame missing: {ir_left_path}"
                    )
                if not ir_right_path.is_file():
                    raise FileNotFoundError(
                        f"recording IR right frame missing: {ir_right_path}"
                    )
            optional_ir_pair = (
                self.has_ir_stereo
                and ir_left_path.is_file()
                and ir_right_path.is_file()
            )
            refs.append(
                RecordedRgbdFrameRef(
                    step=step,
                    ...
                    ir_left_path=ir_left_path
                    if self.requires_ir or optional_ir_pair
                    else None,
                    ir_right_path=ir_right_path
                    if self.requires_ir or optional_ir_pair
                    else None,
                )
            )
        if not refs:
            raise ValueError(
                f"recording has no complete fake-live frames for camera {self.camera_index}"
            )
        return refs
    ...
    def _load_color_bgr(self, path: Path) -> np.ndarray:
        ...
        try:
            ...
            raise ValueError(
                f"failed to load recording color frame {path}: {exc}"
            ) from exc
    ...
    def _load_depth_u16(self, path: Path) -> np.ndarray:
        ...
        try:
            ...
            raise ValueError(
                f"failed to load recording depth frame {path}: {exc}"
            ) from exc
    ...
    def _load_gray_u8(self, path: Path) -> np.ndarray:
        ...
        try:
            ...
            raise ValueError(
                f"failed to load recording IR frame {path}: {exc}"
            ) from exc
...
def _start_realsense_pipeline(args: argparse.Namespace) -> RealtimeCameraRuntime:
    ...
    if args.depth_source == "ffs":
        config.enable_stream(
            rs.stream.infrared, 1, width, height, rs.format.y8, int(args.fps)
        )
        config.enable_stream(
            rs.stream.infrared, 2, width, height, rs.format.y8, int(args.fps)
        )
    if args.depth_source == "realsense":
        config.enable_stream(
            rs.stream.depth, width, height, rs.format.z16, int(args.fps)
        )
        ...
        if exposure is not None or gain is not None:
            ...
            if exposure is not None:
                if not color_sensor.supports(rs.option.exposure):
                    raise RuntimeError(
                        "RealSense RGB sensor does not support exposure control"
                    )
                color_sensor.set_option(rs.option.exposure, float(exposure))
            if gain is not None:
                if not color_sensor.supports(rs.option.gain):
                    raise RuntimeError(
                        "RealSense RGB sensor does not support gain control"
                    )
