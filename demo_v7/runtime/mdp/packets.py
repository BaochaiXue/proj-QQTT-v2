...
from qqtt.tracking.backends.point_tracker_adapter import TRACKER_BACKEND_TAPNEXTPP

from demo_v7.runtime.shape_prior import warmup as shape_prior_warmup
from demo_v7.runtime.mdp.constants import CAMERA_COLOR_FRAME
...
class MaskedPcdPacket:
    ...
    dropped_capture_frames: int  # Capture frames skipped before this packet.
    timing: PipelineTiming  # Accumulated latency measurements for this frame.
    coordinate_frame: str = CAMERA_COLOR_FRAME  # Coordinate frame of all XYZ arrays.
    source_timestamp_s: float | None = None  # Original capture timestamp in seconds.
    source_frame_index: int | None = None  # Zero-based index in the source stream.
    source_step: int | None = None  # Original recording step or filename stem.
...
@dataclass(frozen=True)
class TrackerQuerySet:
    """Frozen chunk-0 tracker query selection plus its shrinking visibility.

    The six query arrays are parallel (one row per query) and frozen once the
    first trackable frame arrives; ``consistent_visible`` is the only mutable
    member and only ever shrinks (in-place AND per frame).
    """

    points_yx: np.ndarray  # Frozen query image coordinates, shape (Q, 2).
    rgb_u8: np.ndarray  # Frozen query-point RGB colors, shape (Q, 3).
    is_object: np.ndarray  # Object-class flags, shape (Q,).
    is_controller: np.ndarray  # Controller-class flags, shape (Q,).
    target_id: np.ndarray  # Target IDs, shape (Q,).
    controller_instance_id: np.ndarray  # Controller instance IDs, shape (Q,).
    consistent_visible: np.ndarray  # Queries still visible under lift gates.

    def and_update_consistent_visible(self, lift_valid: np.ndarray) -> int:
        """AND per-query lift validity into the running visibility; return count."""
        full = np.zeros_like(self.consistent_visible, dtype=bool)
        fitted_count = min(len(lift_valid), len(full))
        full[:fitted_count] = lift_valid[:fitted_count]
        np.logical_and(self.consistent_visible, full, out=self.consistent_visible)
        return int(np.count_nonzero(self.consistent_visible))
...
class TrackerMarkerPacket:
    ...
    coordinate_frame: str = CAMERA_COLOR_FRAME  # Coordinate frame of marker_xyz_m.
...
class PcdBuildResult:
    pcd_packet: MaskedPcdPacket  # Materialized point-cloud packet.
...
class PairedBuildResult:
    ...
    def __post_init__(self) -> None:
        ...
        seq = int(self.seq)
        pcd_seq = int(self.pcd_result.pcd_packet.seq)
