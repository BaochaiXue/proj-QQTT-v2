...
class HeadlessCaptureWriter:
    ...
    def _write_metadata_payload(self, payload: dict[str, Any]) -> None:
        ...
        atomic_json_dump(payload, self.metadata_path)
    ...
    def write_shape_prior_result(
        ...
        case_dir = result.metadata.get("shape_prior_case_dir")
        if not case_dir:
            raise RuntimeError(
                "ready shape-prior result is missing shape_prior_case_dir"
            )
        values = {
    ...
    def write_input_frame(self, packet: FramePacket) -> None:
        ...
        if self.write_input_rgb_timeline or not self.prepared_only:
            bgr_to_pil_rgb(packet.color_bgr).save(rgb_path)
    ...
    def write_pcd(
        ...
        *,
        ...
        if not self.prepared_only:
            bgr_to_pil_rgb(mask_packet.color_bgr).save(rgb_path)
            np.save(
                depth_path,
