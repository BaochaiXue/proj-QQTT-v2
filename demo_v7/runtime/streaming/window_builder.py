...
from demo_v7.runtime.streaming.data_payload import ChunkDataWindow
from demo_v7.runtime.streaming.online_frame_archive import OnlineFrameArchiveError
from demo_v7.runtime import phystwin_strict_product as strict
from demo_v7.runtime.streaming import window_observations
...
def _track_input_with_session_query_schema(
    *,
    ...
    ``session_query_schema`` is the ``ChunkStreamSession``-owned schema dict.
    ...
    if "query_ids" in session_query_schema:
...
def _chunk_data_window_from_prepared_frames(
    ...
    *,
    ...
    chunk_index: int,
    borrow_frames: Sequence[strict.PreparedPhysTwinFrame] = (),
) -> ChunkDataWindow:
    ...
    the design_spec.md tracking state machine through the session-lived
    ``session.tracking_runtime`` with the session-frozen query schema.
    ``borrow_frames`` extend the motion-consistency domain but are excluded
    from the published window.
    """
    if not frames:
    ...
    for frame in list(frames) + list(borrow_frames):
    ...
    track_input = _track_input_with_session_query_schema(
        ...
        session_query_schema=session.session_query_schema,
    )
    ...
    track_process = session.tracking_runtime.process_window(
        track_input,
