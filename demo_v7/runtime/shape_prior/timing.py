...
from demo_v7.runtime.utils.atomic_io import atomic_json_dump
...
class StageProfileRun:
    """One stage CLI's WAITING/GO/COMPLETED profile lifecycle.

    Owns the stage name, execution mode, timing dict, and readiness stamp;
    ``active_fields`` is the single source of the ``total_ms`` accounting rule
    (the GO idle wait and lifetime bookkeeping fields are never counted).
    """
    ...
    def __init__(
        ...
        *,
        ...
        process_started_s: float,
    ) -> None:
        """Initialize StageProfileRun."""
        self.stage = str(stage)
        self.profile_json = profile_json
        self.execution_mode = "prewarmed" if wait_signal else "cold"
        self.timing_ms = timing_ms
        self.active_fields = tuple(active_fields)
        self.ready_wall_time_s: float | None = None
        self._process_started_s = float(process_started_s)

    def _write(self, status: str) -> None:
        self.timing_ms["total_ms"] = float(
            sum(self.timing_ms[field] for field in self.active_fields)
        )
        self.timing_ms["process_lifetime_ms"] = elapsed_ms(self._process_started_s)
        write_stage_profile(
            self.profile_json,
            stage=self.stage,
            status=status,
            ...
            ready_wall_time_s=self.ready_wall_time_s,
        )

    def write_waiting(self) -> None:
        """Stamp readiness and write the pre-GO WAITING snapshot."""
    ...
    def write_completed(self) -> None:
        """Write the final COMPLETED snapshot."""
        self._write(STAGE_PROFILE_STATUS_COMPLETED)


def pre_submit_timing(
    frame0: Any,
    *,
    ...
    ``frame0`` is a ``shape_prior.case.ShapePriorFrame0Request``; it is
    typed loosely so this timing module needs no case-writer import.
    """
    milestones = (
