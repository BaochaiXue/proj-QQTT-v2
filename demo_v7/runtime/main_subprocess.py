...
from demo_v7.runtime.mdp.constants import DEPTH_BACKEND_TO_DEPTH_SOURCE
...
from demo_v7.runtime.main_options import (
...
def build_visualizer_command(
    ...
    *,
    ...
    if visualizer_uses_web_frontend(args):
        ...
        return [
            *python_command_prefix(getattr(args, "visualizer_conda_env", None)),
            str(Path("demo_v7") / "runtime" / "visualization" / "visualize_track_web.py"),
            "--online-dir",
    ...
    command = [
        *python_command_prefix(getattr(args, "visualizer_conda_env", None)),
        str(Path("demo_v7") / "runtime" / "visualization" / "visualize_track.py"),
        "--layout",
        layout,
        "--online-dir",
        str(resolve_online_dir(args)),
...
def build_main_data_processing_command(
    ...
    *,
    ...
    # demo_v7 always retargets command[1] at its own camera service; the
    # default only documents the expected argv shape.
    script = Path("demo_v7") / "service" / "camera_service.py"
