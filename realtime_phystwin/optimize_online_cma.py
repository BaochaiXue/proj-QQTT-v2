...
def wait_for_initial_frames(reader, buffer, segment_len, poll_sec):
    ...
    while buffer.num_frames < segment_len:
        ...
        logger.info(
            "[CMA-Online]: waiting for initial frames, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Export the best CMA candidate every N zero-order iterations.",
    )
    parser.add_argument(
    ...
    logger.set_log_file(path=base_dir, name="online_optimize_cma_log")
    logger.info(f"[CMA-Online]: online_dir={online_dir}")
    logger.info(f"[CMA-Online]: static_data_path={static_data_path}")
    ...
    optimizer.optimize_online_batched(
