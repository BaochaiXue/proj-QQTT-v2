# Demo v5.1 Environment Files

This folder contains the install and verification material for Demo v5.1. Demo v5.1
...
conda run -n demo_2_max --no-capture-output \
  python demo_v5_1/env/check_demo_v5_env.py --role main --require-cuda

conda run -n phystwin-max --no-capture-output \
  python demo_v5_1/env/check_demo_v5_env.py --role shape-prior --require-cuda
```
...
```bash
bash demo_v5_1/env/install_demo_v5_env.sh create
```

Or run the steps manually:

```bash
conda env create -f demo_v5_1/env/environment-demo-v5-main.yml
conda run -n demo_2_max --no-capture-output \
  python -m pip install -r demo_v5_1/env/requirements-demo-v5-main.txt

conda env create -f demo_v5_1/env/environment-demo-v5-shape-prior.yml
conda run -n phystwin-max --no-capture-output \
  python -m pip install -r demo_v5_1/env/requirements-demo-v5-shape-prior.txt
```

Update existing environments:

```bash
bash demo_v5_1/env/install_demo_v5_env.sh update
```

Or run the steps manually:

```bash
conda env update -f demo_v5_1/env/environment-demo-v5-main.yml --prune
conda run -n demo_2_max --no-capture-output \
  python -m pip install -r demo_v5_1/env/requirements-demo-v5-main.txt

conda env update -f demo_v5_1/env/environment-demo-v5-shape-prior.yml --prune
conda run -n phystwin-max --no-capture-output \
  python -m pip install -r demo_v5_1/env/requirements-demo-v5-shape-prior.txt
```
...
`bash demo_v5_1/env/install_demo_v5_env.sh update` or install a CUDA toolkit whose
