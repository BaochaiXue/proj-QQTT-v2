"""Demo v5.1 realtime tracking state machine (see demo_v5_1/design_spec.md).
...
from demo_v5_1 import ffs, segment
...
class TrackingRuntime:
    ...
    def _check_frozen_identity(self, window: Mapping[str, np.ndarray]) -> None:
        ...
        if not np.array_equal(self._query_ids, np.asarray(window["query_ids"], dtype=np.int64)):
            raise ValueError("Demo v5.1 session query_ids changed across chunks")
