"""Convert Demo v6 headless capture rows into realtime chunks.
...
from demo_v6.chunk_data_payload import (
    ChunkDataWindow,
...
def _trim_warmup_delayed_rows(rows: Sequence[Mapping[str, Any]]) -> WarmupTrimResult:
    ...
    Demo v6 writes ``input_frames.jsonl`` from camera start, but
...
def _apply_radius_outlier_to_mask_frame(
    ...
    *,
    ...
    # corresponding mask pixels. Demo v6 does the same per frame in memory.
    normalized = strict.normalize_processed_mask_frame(frame)
    if not bool(enabled):
        return normalized
...
def _shape_points_for_chunk(
    ...
    *,
    ...
    Demo v6 keeps capture realtime, but ``final_data.pkl`` must contain
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise RuntimeError(
                "shape prior is required for Demo v6 final_data chunks, "
...
def _track_input_with_session_query_schema(
    *,
    ...
    # visible track pixels into world-space PCD samples. Demo v6 also pins
    ...
    if not np.array_equal(session_query_schema["query_ids"], track_input["query_ids"]):
        raise ValueError("Demo v6 session query_ids changed across chunks")
    if not np.array_equal(
        session_query_schema["query_semantic_labels"],
        track_input["query_semantic_labels"],
    ):
        raise ValueError("Demo v6 session query_semantic_labels changed across chunks")
    return track_input
...
def _chunk_data_window_from_prepared_frames(
    ...
    *,
    ...
    This is the current v6 realtime path. The camera process already wrote
...
def _write_chunk_from_rows(
    *,
    ...
    if prepared and len(prepared) == len(rows) and prepared_count == len(rows):
        ...
        # Fallback for historical captures where Demo v6 has to reconstruct
    ...
    if asap_runtime is not None:
        # design_spec_v6.md: the ASAP augmenter runs live at materialization,
...
def write_chunk_data_from_headless_capture(
    ...
    *,
    ...
    serials = metadata.get("serial_numbers") or ["demo-v6-single-camera"]
    ...
    # happens once per case; Demo v6 treats the live stream as one case.
    tracking_runtime = tracking.TrackingRuntime()
    # design_spec_v6.md: the ASAP augmenter is session-lived like the tracking
...
def stream_chunk_data_from_headless_capture(
    ...
    *,
    ...
    tracking_runtime = tracking.TrackingRuntime()
    # design_spec_v6.md: session-lived ASAP augmenter; initializes from the
