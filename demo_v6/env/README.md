# Demo v6 Environment Files

This folder contains the install and verification material for Demo v6. Demo v6
...
conda run -n demo_2_max --no-capture-output \
  python demo_v6/env/check_demo_v6_env.py --role main --require-cuda

conda run -n phystwin-max --no-capture-output \
  python demo_v6/env/check_demo_v6_env.py --role shape-prior --require-cuda
```
...
```bash
bash demo_v6/env/install_demo_v6_env.sh create
```

Or run the steps manually:

```bash
conda env create -f demo_v6/env/environment-demo-v6-main.yml
conda run -n demo_2_max --no-capture-output \
  python -m pip install -r demo_v6/env/requirements-demo-v6-main.txt

conda env create -f demo_v6/env/environment-demo-v6-shape-prior.yml
conda run -n phystwin-max --no-capture-output \
  python -m pip install -r demo_v6/env/requirements-demo-v6-shape-prior.txt
```

Update existing environments:

```bash
bash demo_v6/env/install_demo_v6_env.sh update
```

Or run the steps manually:

```bash
conda env update -f demo_v6/env/environment-demo-v6-main.yml --prune
conda run -n demo_2_max --no-capture-output \
  python -m pip install -r demo_v6/env/requirements-demo-v6-main.txt

conda env update -f demo_v6/env/environment-demo-v6-shape-prior.yml --prune
conda run -n phystwin-max --no-capture-output \
  python -m pip install -r demo_v6/env/requirements-demo-v6-shape-prior.txt
```
...
`bash demo_v6/env/install_demo_v6_env.sh update` or install a CUDA toolkit whose
