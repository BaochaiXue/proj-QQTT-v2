...
MAIN_ENV="${DEMO_V6_MAIN_ENV:-demo_2_max}"
SHAPE_ENV="${DEMO_V6_SHAPE_PRIOR_ENV:-phystwin-max}"
MODE="${1:-update}"
SAM3_PACKAGE_REF="${DEMO_V6_SAM3_PACKAGE_REF:-sam3==0.1.4}"
PYTORCH3D_SOURCE="${DEMO_V6_PYTORCH3D_SOURCE:-${HOME}/external/pytorch3d-demo-v6-cu126}"
PYTORCH3D_REF="${DEMO_V6_PYTORCH3D_REF:-v0.7.9}"

usage() {
    cat <<'EOF'
Install or update Demo v6 conda environments.

Usage:
  bash demo_v6/env/install_demo_v6_env.sh [update|create|check]

Environment variables:
  DEMO_V6_MAIN_ENV         default: demo_2_max
  DEMO_V6_SHAPE_PRIOR_ENV  default: phystwin-max
  DEMO_V6_SAM3_PACKAGE_REF default: sam3==0.1.4
  DEMO_V6_PYTORCH3D_SOURCE default: ~/external/pytorch3d-demo-v6-cu126
  DEMO_V6_PYTORCH3D_REF    default: v0.7.9
...
install_main_compiled_extras() {
    ...
    local build_env=(
        ...
        FORCE_CUDA=1
        TORCH_CUDA_ARCH_LIST="${DEMO_V6_TORCH_CUDA_ARCH_LIST:-8.9}"
        MAX_JOBS="${DEMO_V6_BUILD_MAX_JOBS:-8}"
    )
    ensure_pytorch3d_source
...
run_checks() {
    conda run -n "${MAIN_ENV}" --no-capture-output \
        python "${REPO_ROOT}/demo_v6/env/check_demo_v6_env.py" --role main --require-cuda
    conda run -n "${SHAPE_ENV}" --no-capture-output \
        python "${REPO_ROOT}/demo_v6/env/check_demo_v6_env.py" --role shape-prior --require-cuda
}
...
main() {
    case "${MODE}" in
        ...
        check)
            ...
            ;;
        update|create)
            create_or_update_env "${MAIN_ENV}" "${SCRIPT_DIR}/environment-demo-v6-main.yml"
            install_pip_extras "${MAIN_ENV}" "${SCRIPT_DIR}/requirements-demo-v6-main.txt"
            install_main_no_deps_extras
            install_main_compiled_extras
            create_or_update_env "${SHAPE_ENV}" "${SCRIPT_DIR}/environment-demo-v6-shape-prior.yml"
            install_pip_extras "${SHAPE_ENV}" "${SCRIPT_DIR}/requirements-demo-v6-shape-prior.txt"
            install_shape_prior_compiled_extras
            run_checks
            ;;
        *)
