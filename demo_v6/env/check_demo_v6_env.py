#!/usr/bin/env python3
"""Check Demo v6 runtime imports, toolchains, and repo-local assets."""
...
def build_parser() -> argparse.ArgumentParser:
    """Build the Demo v6 environment-check CLI parser."""
    parser = argparse.ArgumentParser(
        description="Check Demo v6 runtime environment and repo-local assets."
    )
...
def main(argv: list[str] | None = None) -> int:
    """Run the requested Demo v6 environment checks."""
    args = build_parser().parse_args(argv)
    errors: list[str] = []
    role = str(args.role)
    if role in {"main", "all"}:
        print("[check] Demo v6 main environment")
    ...
    if role in {"shape-prior", "all"}:
        print("[check] Demo v6 shape-prior worker environment")
    ...
    if errors:
        print("[fail] Demo v6 environment check failed:")
        for error in errors:
            print(f"  - {error}")
        return 1
    print("[ok] Demo v6 environment check passed")
    return 0
