...
from demo_v6.phystwin_strict_product import PreparedPhysTwinFrame
from demo_v6.utils.atomic_io import atomic_json_dump, atomic_pickle_dump
...
def _atomic_write_bytes(path: Path, blob: bytes) -> None:
    """Write bytes through a fsync'd temp file so readers never see partial
    files — metadata.json is fsync'd, so the frame files it points at must
...
class OnlineFrameArchive:
    ...
    chunks/ or manifest.json; calibration initializes lazily from the first
    archived chunk's capture metadata, mirroring the session-lived tracking
    and ASAP runtimes.
    """

    def __init__(self, *, base_path: str | Path, case_name: str, fps: int) -> None:
    ...
    def _initialize_calibration(
        self, metadata: Mapping[str, Any], *, serial_number: str
    ) -> dict[str, Any]:
        ...
        calibration = {
            ...
            "width": int(width),
            "height": int(height),
            "serial_number": str(metadata.get("serial") or serial_number),
        }
        ...
        *,
        ...
        if frame.depth_mm_u16 is None:
            raise OnlineFrameArchiveError(
                ...
                "capture with the current demo_v6"
            )
        ...
        _atomic_write_bytes(
            self.color_dir / f"{online_frame_index}.png", png.tobytes()
        )
        depth_path = self.depth_dir / f"{online_frame_index}.npy"
        tmp = depth_path.with_name(depth_path.name + ".tmp")
        with tmp.open("wb") as handle:
        ...
        *,
        ...
        if int(online_start_frame) != int(self.frames_written):
            raise OnlineFrameArchiveError(
                ...
                f"archive has {self.frames_written} frames but the chunk "
        ...
        if source_timestamps_s is not None and len(source_timestamps_s) != len(frames):
            raise OnlineFrameArchiveError(
                f"chunk {chunk_id}: {len(frames)} prepared frames but "
                f"{len(source_timestamps_s)} source_timestamps_s"
            )
            ...
            self._frame_mapping.append(
                {
                    ...
                    "chunk_frame_index": int(local_index),
                    "seq": int(frame.seq),
                    "source_frame_index": source_frame_index,
                    "source_step": (
                        None if frame.source_step is None else int(frame.source_step)
                    ),
                    "source_timestamp_s": (
                        None
                        if source_timestamps_s is None
                        or source_timestamps_s[local_index] is None
                        else float(source_timestamps_s[local_index])
                    ),
                    "color_path": f"color/{CAMERA_INDEX}/{online_frame_index}.png",
                    "depth_path": f"depth/{CAMERA_INDEX}/{online_frame_index}.npy",
                }
            )
        self.frames_written += len(frames)
        return {
            "online_frame_archive_frames": int(self.frames_written),
    ...
    def _write_metadata(self, calibration: Mapping[str, Any]) -> None:
        ...
        metadata = {
            "schema_version": "demo_v6_online_data_v1",
            ...
            "frame_num": int(self.frames_written),
            "fps": int(self.fps),
    ...
    def _write_enhance_metadata(self) -> None:
        ...
        payload = {
            ...
            "demo_version": "demo_v6",
            "case_name": self.case_name,
            "num_frames": int(self.frames_written),
            "color_dir": f"color/{CAMERA_INDEX}",
            "depth_dir": f"depth/{CAMERA_INDEX}",
            "depth_encoding": DEPTH_ENCODING,
