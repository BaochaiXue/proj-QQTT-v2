"""Single-camera shape-prior warmup for Demo v6."""
...
def write_shape_prior_case(
    ...
    *,
    ...
    from demo_v6.chunk_data_stream import (  # noqa: PLC0415
...
class ShapePriorLocalClient:
    ...
    def _stage_env(self) -> dict[str, str]:
        ...
        # Subprocess stages must import demo_v6 both as a package (repo
        # root) and as top-level modules (demo_v6/), pinned to the warmup
        # GPU via CUDA_VISIBLE_DEVICES.
        env = os.environ.copy()
        env["CUDA_VISIBLE_DEVICES"] = str(self.cuda_visible_devices)
        python_path = [str(REPO_ROOT), str(REPO_ROOT / "demo_v6")]
    ...
    def request_shape_prior(self, frame0: ShapePriorFrame0Request) -> ShapePriorResult:
        ...
        from demo_v6 import sam31_image_segmentation  # noqa: PLC0415
