#!/usr/bin/env python3
"""Demo v6 realtime orchestration entrypoint.

This runner owns process boundaries, GPU routing, and artifact publication. The
actual camera/tracker stack runs in ``demo_v6/main_data_processing.py``;
...
from demo_v6.chunk_data_stream import (
...
def load_default_config(path: Path = DEFAULT_CONFIG_PATH) -> dict[str, object]:
    """Load Demo v6 defaults from YAML."""
...
# Defaults below describe the current Demo v6 realtime path.
...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for Demo v6 realtime orchestration."""
    parser = argparse.ArgumentParser(
        description=(
            "Demo v6 realtime data_process_sam3d runner. It turns Demo v6 "
    ...
    parser.add_argument(
        ...
        help="Camera source mode used when Demo v6 launches its own capture.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional Demo v6 fake-live pacing FPS. When omitted, Demo v6 uses "
            "--replay-fps; Demo v6 output metadata/window math still use --replay-fps."
        ),
    )
    parser.add_argument(
        "--fake-live-case",
        ...
        help="Raw data_collect case folder passed to Demo v6 fake-live replay.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional strict lossless replay backlog window passed to Demo v6. "
            "Omit it to keep Demo v6 defaults."
        ),
    )
    parser.add_argument(
        "--camera-fps",
        ...
        help=(
            "RealSense capture FPS passed to Demo v6 live camera. "
    ...
    parser.add_argument(
        ...
        help="Manual RealSense RGB exposure passed to Demo v6 live camera.",
    )
    parser.add_argument(
        "--camera-color-gain",
        type=float,
        default=DEFAULT_CAMERA_COLOR_GAIN,
        help="Manual RealSense RGB gain passed to Demo v6 live camera.",
    )
    ...
    # becoming part of the v6 contract.
    parser.add_argument(
        "--camera-headless-prepared-only",
        dest="camera_headless_prepared_only",
        action="store_true",
        help="Ask Demo v6 to write only prepared PhysTwin frames needed by Demo v6 chunking.",
    )
    parser.add_argument(
        ...
        action="store_false",
        help=(
            "Keep Demo v6 legacy per-frame headless artifacts in addition to "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Write input_rgb/*.png and input_frames.jsonl for the Demo v6 "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Headless capture directory for the Demo v6 realtime subprocess.",
    )
    parser.add_argument(
        "--source-headless-capture",
        type=Path,
        default=None,
        help="Existing Demo v6 headless capture directory to chunk without launching capture.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Keep SAM3D shape-prior warmup enabled for Demo v6 capture.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "How long Demo v6 waits for required shape-prior structure points "
    ...
    parser.add_argument(
        ...
        help="window launches the Demo v6 visualizer.",
    )
    parser.add_argument(
        "--visualizer-layout",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print resolved Demo v6 contract and exit.",
    )
    return parser
...
def resolve_camera_source_replay_fps(args: argparse.Namespace) -> float:
    ...
    if not math.isfinite(fps) or fps <= 0.0:
        raise ValueError("Demo v6 source replay fps must be positive")
    return fps
    ...
    *,
    ...
    command = [
        *_python_command_prefix(getattr(args, "visualizer_conda_env", None)),
        str(Path("demo_v6") / "visualize_track.py"),
        "--layout",
        layout,
        "--online-dir",
...
def prepare_realtime_output_for_new_run(
    ...
    *,
    ...
    """Remove stale generated outputs before writing fixed Demo v6 paths."""
    base = Path(base_path)
    cleanup_paths = {
...
def build_main_data_processing_command(
    ...
    *,
    ...
    script = Path("demo_v6") / "main_data_processing.py"
    camera_source_replay_fps = resolve_camera_source_replay_fps(args)
    ...
    # Demo v6 chunks are bounded by the chunk publisher, not by the camera
    ...
    duration_s = 0.0
    # This is the only v6 camera/tracker entrypoint. It writes prepared
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run Demo v6 offline conversion or live/fake-live orchestration."""
    parser = build_parser()
