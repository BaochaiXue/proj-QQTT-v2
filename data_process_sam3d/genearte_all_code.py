...
DEFAULT_ROOTS: tuple[Path, ...] = (
    SCRIPT_DIR,
    SCRIPT_DIR.parent / "data_process" / "shape_prior.py",
)
...
def collect_files(root: Path, suffixes: Iterable[str], ignore_dirs: Sequence[str]) -> list[Path]:
    ...
    If root is a single file, it is returned when the suffix matches.
    """
    ...
    if root.is_file():
        if root.resolve() != SCRIPT_PATH and root.suffix.lower() in suffix_set:
            return [root]
        return []
...
def write_markdown(files_with_roots: Iterable[tuple[Path, Path]], output_path: Path) -> None:
    """Write a Markdown file containing each source file surrounded by fenced code blocks.

    files_with_roots: iterable of (root, path) pairs so we can stitch multiple roots together.
    """
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for root, path in files_with_roots:
            ...
            handle.write(f"{root.name}/{rel_path.as_posix()}\n")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to this script directory (data_process_sam3d) and data_process/shape_prior.py."
        ),
    )
    parser.add_argument(
        "--output",
    ...
    files_with_roots: list[tuple[Path, Path]] = []
    for root in roots:
        if root.is_file():
            if root.suffix.lower() not in SUFFIX_TO_LANG:
                raise ValueError(f"Unsupported file type for {root}")
            files_with_roots.append((root.parent.resolve(), root))
            continue

        files = collect_files(root, SUFFIX_TO_LANG.keys(), args.ignore)
        files_with_roots.extend((root, f) for f in files)

    write_markdown(files_with_roots, args.output)
