...
def loadCam(args, id, cam_info, resolution_scale, is_nerf_synthetic, is_test_dataset):
    ...
    if cam_info.depth is not None:
        ...
        depth = torch.nn.functional.interpolate(
            depth, (resolution[1], resolution[0]), mode="bilinear", align_corners=True
        )
    ...
    if cam_info.normal is not None:
        ...
        normal = torch.nn.functional.interpolate(
            normal, (resolution[1], resolution[0]), mode="nearest"
        )
    ...
    if cam_info.occ_mask is not None:
        ...
        occ_mask = torch.nn.functional.interpolate(
            occ_mask, (resolution[1], resolution[0]), mode="nearest"
        )
